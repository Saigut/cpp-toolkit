                                      ...
                                      const std::string& user_pass)
{
    ...
    auto cb = [&](std::error_code ec){
        if (ec) {
            ...
            log_info("Register failed! User id: %s", user_id.c_str());
            prom.set_value(0);
        }
    };
...
{
    auto find_rst = m_operation.m_user_info.user_find(user_id);
    if (!find_rst) {
    ...
    if (0 != m_operation.m_user_session.add(user_id, client_ip, client_port)) {
...
std::shared_ptr<prod_im_cont_list> prod_im_s_mod_main::get_contact_list(const std::string& user_id)
{
    ...
    auto rst = m_operation.m_user_session.find(user_id);
    if (!rst) {
        log_error("User is not login! User id: %s", user_id.c_str());
        return nullptr;
    }
    return m_operation.m_user_info.user_contact_get_list(user_id);
}
                                    ...
                                    const std::string& contact_name)
{
    ...
    return m_operation.m_user_info.user_contact_add(user_id, contact_id, contact_name);
}
                                    ...
                                    const std::string& contact_id)
{
    ...
    m_operation.m_user_info.user_contact_del(user_id, contact_id);
    return 0;
}
...
{
    auto rst = m_operation.m_user_session.find(sender_id);
    if (!rst) {
        log_error("User does not login! User id: %s", sender_id.c_str());
        return;
    }
    rst = m_operation.m_user_session.find(receiver_id);
    if (!rst) {
        log_error("Receiver does not login! Receiver id: %s", receiver_id.c_str());
        return;
    }
    m_operation.m_chat_msg_relay.relay_msg(rst->client_ip, rst->client_port, sender_id, receiver_id, chat_msg);
}
