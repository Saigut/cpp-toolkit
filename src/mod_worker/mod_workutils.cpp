...
namespace WorkUtils {
    int TcpSocketConnector_Asio::connect(const std::string& addr_str, uint16_t port) {
        Work_NetTcpConnect work_connect(m_consignor, m_socket_to_server, addr_str, port);
    ...
    int TcpSocketConnector_Asio::read(char *recv_buf, size_t buf_sz, size_t &recv_data_sz) {
        Work_NetTcpIn work_in(m_consignor, m_socket_to_server, recv_buf, buf_sz);
    ...
    int TcpSocketConnector_Asio::write(char *str_buf, size_t str_len) {
        Work_NetTcpOut work_out(m_consignor, m_socket_to_server, str_buf, str_len);
